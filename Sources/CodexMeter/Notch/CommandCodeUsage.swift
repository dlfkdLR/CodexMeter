...
/// Parses Command Code's `/alpha` billing payload — the same documents the
/// desktop app's `fetchUsageData` asks for:
///
...
/// Five-hour and weekly windows ride in the tooltip when the cap is known. A
/// `resetAt` of 0 is absence, not 1970.
///
/// Ported from the MIT-licensed Codenotch (`CommandCodeUsage`).
enum CommandCodeUsage {
    ...
    static func windows(summaryJSON: String,
        ...
        guard cap > 0 else {
            throw NotchProviderError.nothingMetered("Command Code has nothing metered on this account yet")
        }

        var windows: [LimitWindow] = [
            LimitWindow(id: "monthly", label: "Monthly limit",
                        usedFraction: used / cap, resetsAt: periodEnd)
        ]
    ...
    private static func window(_ any: Any?, id: String, label: String) -> LimitWindow? {
        ...
        return LimitWindow(id: id, label: label, usedFraction: used / cap,
