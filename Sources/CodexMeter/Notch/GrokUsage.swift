...
/// Parses Grok CLI's `GET /v1/billing?format=credits`, recorded from a live
/// SuperGrok session. The credits payload is the ring: a weekly Grok Build
/// allowance, the one number this account's own endpoint actually states.
///
/// Grok's account charge date is not in this response — the unformatted
/// `/billing` payload's `billingPeriodEnd` is a calendar-month usage ledger,
/// not a bill, so showing one would mean guessing at a fact the endpoint does
/// not state.
///
/// Ported from the MIT-licensed Codenotch (`GrokUsage`).
enum GrokUsage {
    static func windows(creditsJSON: String) throws -> [LimitWindow] {
        ...
        if let fraction = percent(credits["creditUsagePercent"]) {
            ...
            for product in products {
                ...
                // wire product name here left a valid bar in the tooltip and a
                // dash on the cell.
                windows.append(LimitWindow(
        ...
        // usage lands. Grok's own `/usage` still shows this as a "Weekly limit"
        // bar at 0%, so mirror it rather than reporting the account as unmetered.
        if windows.isEmpty,
        ...
        guard !windows.isEmpty else {
            throw NotchProviderError.nothingMetered("Grok has nothing metered on this account yet")
        }
        return windows
    }
