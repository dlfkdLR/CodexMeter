...
/// Parses `GET https://opencode.ai/zen/go/v1/usage` — the account-wide Go plan
/// windows, the same figures the OpenCode dashboard shows:
///
/// ```json
/// {"usage":{
...
/// formatter refuses, so both fractional and plain forms are tried.
///
/// Ported from the MIT-licensed Codenotch (`OpenCodeUsage`).
enum OpenCodeUsage {
    ...
    static func windows(fromJSON json: String, now: Date = Date()) throws -> [LimitWindow] {
        ...
        guard !out.isEmpty else { throw NotchProviderError.badResponse(status: 0) }
        return out
    }
